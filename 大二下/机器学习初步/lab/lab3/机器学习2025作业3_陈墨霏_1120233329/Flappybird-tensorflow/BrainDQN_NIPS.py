...
class BrainDQN:
	...
	def createQNetwork(self):
		...
		if checkpoint and checkpoint.model_checkpoint_path:
			...
			if os.path.exists('saved_networks/replayMemory.pkl'):
				with open('saved_networks/replayMemory.pkl', 'rb') as f:
					self.replayMemory = pickle.load(f)
			# 修改：将timeStep和epsilon也加以保存，以便后续重新加载
			if os.path.exists('saved_networks/metadata.json'):
				with open('saved_networks/metadata.json', 'r') as f:
	...
	def trainQNetwork(self):
		...
		if self.timeStep % 10000 == 0:
			...
			with open('saved_networks/replayMemory.pkl', 'wb') as f:
				pickle.dump(self.replayMemory, f)
			with open('saved_networks/metadata.json', 'w') as f:
				metadata = {
