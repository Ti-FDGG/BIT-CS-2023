...
CREATE TABLE Part (
    ...
    part_name VARCHAR(50), -- 零件名称
    mfgr VARCHAR(50), -- 制造商
    size INT, -- 零件尺寸，可以为NULL
...
可以看到，创建的基本表中包含了各个表的主键和外键约束，确保了数据的完整性和一致性。这些约束不仅能够有效防止数据冗余和不一致，还能够通过外键关系建立表与表之间的关联，为后续的查询和操作提供了便利。此外，合理的字段设计和数据类型选择也为数据库的高效运行奠定了基础。

== 实验收获与体会

通过本次实验，我熟悉了MySQL数据库的安装与配置过程，掌握了基本的数据库操作方法，包括创建数据库、定义表结构及设置主外键约束等。借助DataGrip工具，我体验到了图形化界面带来的便捷性，同时也加深了对SQL语句的理解和应用能力。

此外，本实验让我认识到数据库设计的重要性。合理的表结构设计和字段约束不仅能提高数据存储的效率，还能确保数据的完整性和一致性。这为后续的数据库开发和复杂查询操作奠定了坚实的基础。
